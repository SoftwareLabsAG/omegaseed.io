#!/usr/bin/env bash
# =============================================================================
#  omega-update.sh — holt eine neue Version von GitHub und schaltet sie live,
#  aber nur wenn BEIDE Signaturen gegen die fest hinterlegten Schluessel passen.
#
#  Der Server vertraut GitHub nicht. GitHub ist nur der Transportweg; die
...
REPO="${OMEGA_REPO:-SoftwareLabsAG/omegaseed.io}"
...
# ---- 2. Welche Version bietet die Quelle? ----------------------------------
if [[ -n "$SOURCE" ]]; then
  TAG="$(cat "$SOURCE/TAG")"
...
NEED=("$ZIP" "SHA256SUMS.txt" "SHA256SUMS.txt.minisig" "SHA256SUMS.txt.mldsa")
if [[ -z "$SOURCE" ]]; then
  BASE="https://github.com/$REPO/releases/download/$TAG"
  for f in "${NEED[@]}"; do
    curl -fsSL --max-time 300 "$BASE/$f" -o "$TMP/$f" || fail "Download fehlgeschlagen: $f"
  done
fi
for f in "${NEED[@]}"; do
